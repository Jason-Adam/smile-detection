...
class RetryTimeoutException(Exception):
    ...
    def __str__(self):
        return (
            "Task has timed out, task name: {0}, total retry count: {1}, total wait time: {2}, "
            "jitter enabled: {3}, retry errors: {4}".format(
                self.task_name,
                self.max_attempts,
        ...
        def retry_impl(*args, **kwargs):
            ...
            while have_tried < max_attempts:
                try:
                    ...
                    going_to_sleep_for = min(
                        max_delay, multiplier * (backoff_coefficient ** have_tried)
                    )
                    if enable_jitter:
                    ...
                    runtime_logger.warn(
                        "Retrying [{0}], going to sleep for {1} seconds, exception stacktrace:\n{2}".format(
                            task_to_try.__name__, duration, traceback.format_exc()
                        )
                    )
                    time.sleep(duration)
