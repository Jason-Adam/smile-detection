...
WorkItem = collections.namedtuple(
    "WorkItem", ["invocation_id", "payload", "client_context"]
)
GetWorkResultOutput = collections.namedtuple(
    "GetWorkResultOutput", ["payload", "func_err"]
)
...
class IPCClient:
    ...
    def post_work(
        ...
        runtime_logger.info(
            "Posting work for function [{}] to {}".format(function_arn, url)
        )
    ...
    def get_work(self, function_arn):
        ...
        runtime_logger.info(
            "Getting work for function [{}] from {}".format(function_arn, url)
        )
        ...
        runtime_logger.info(
            "Got work item with invocation id [{}]".format(invocation_id)
        )
        return WorkItem(
    ...
    def post_work_result(self, function_arn, work_item):
        ...
        runtime_logger.info(
            "Posting work result for invocation id [{}] to {}".format(
                work_item.invocation_id, url
            )
        )
        ...
        runtime_logger.info(
            "Posted work result for invocation id [{}]".format(work_item.invocation_id)
        )
    ...
    def post_handler_err(self, function_arn, invocation_id, handler_err):
        ...
        runtime_logger.info(
            "Posting handler error for invocation id [{}] to {}".format(
                invocation_id, url
            )
        )

        payload = json.dumps({"errorMessage": handler_err}).encode("utf-8")
        ...
        runtime_logger.info(
            "Posted handler error for invocation id [{}]".format(invocation_id)
        )
    ...
    def get_work_result(self, function_arn, invocation_id):
        ...
        runtime_logger.info(
            "Getting work result for invocation id [{}] from {}".format(
                invocation_id, url
            )
        )
