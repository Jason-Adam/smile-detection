...
class GreengrassMessage:
    ...
    def decode(cls, encoded_string):
        if encoded_string:
            try:
                ...
                raise ValueError(
                    'Could not load provided encoded string "{}" as JSON due to exception: {}'.format(
            ...
            try:
                ...
                raise ValueError(
                    ...
                    '"{}" from base64 due to exception: {}'.format(
                        repr(data_map), str(e)
                    )
                )
    ...
    def encode(self):
        try:
            ...
            runtime_logger.exception(e)
            raise ValueError(
                'Could not encode Greengrass Message payload "{}" as base64 due to exception: {}'.format(
        ...
        try:
            ...
            runtime_logger.exception(e)
            raise ValueError(
                'Could not encode Greengrass Message fields "{}" as JSON due to exception: {}'.format(
                    str(self), str(e)
                )
            )
