...
LOCAL_CLOUDWATCH_ENDPOINT = "http://localhost:8000/{version}/cloudwatch/logs/".format(
    version=LOCAL_CLOUDWATCH_API_VERSION
)
...
def wrap_urllib_exceptions(func):
    ...
    def wrapped(*args, **kwargs):
        try:
            ...
            full_stack_trace = traceback.format_exception(
                exc_type, exc_value, exc_traceback
            )
            full_stack_trace.insert(
                0, "Failed to send to Localwatch due to exception:\n"
            )
...
class LocalCloudwatchLogHandler(logging.Handler):
    ...
    def _should_send(self, message, created_time):
        if created_time >= self.oldest_time_stamp + SECONDS_IN_ONE_DAY:
            ...
            self.total_log_event_byte_size + len(message) + LOG_EVENT_OVERHEAD
            > MAX_REQUEST_SIZE
        ):
            return True
    ...
    def emit(self, record):
        ...
        self.events_buffer.append(
            {"timestamp": int(round(record.created * 1000)), "message": msg}
        )
        self.flush()
    ...
    def _send_to_local_cw(self):
        ...
        request = Request(
            LOCAL_CLOUDWATCH_ENDPOINT, json.dumps(request_data).encode("utf-8")
        )
