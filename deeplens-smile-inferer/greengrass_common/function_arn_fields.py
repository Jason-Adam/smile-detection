...
ARN_FIELD_REGEX = "arn:aws:lambda:([a-z]{2}-[a-z]+-\d{1}):(\d{12}):function:([a-zA-Z0-9-_]+)(?::(\$LATEST|[a-zA-Z0-9-_]+))?"
...
class FunctionArnFields:
    ...
    def to_arn_string(self):
        return FunctionArnFields.build_arn_string(
            self.region, self.account_id, self.name, self.qualifier
        )
